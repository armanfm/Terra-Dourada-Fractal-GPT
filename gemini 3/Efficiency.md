...
Instead of simulating memory by resending long conversation histories or documents inside the LLM context window, Terra Dourada performs deterministic semantic recall locally and injects only the minimal relevant fragment into the Gemini 3 prompt.

As a result, token usage remains low, predictable, and independent of the size of the memory or the length of prior interactions. This design directly reduces operational costs, latency, and semantic drift, while preserving contextual accuracy.
...
- Recall is computed locally using a deterministic Rust recall engine
...
Token usage remains stable regardless of the size of the semantic memory (`mind.bin`) or the number of prior interactions.
